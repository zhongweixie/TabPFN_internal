#!/usr/bin/env bash
# Slurm job: H5 ACT λ=0.01, 2xH800 GPU
# Submit: sbatch slurm_h5_l001.sh
# Adjust TACO_SRC, CONDA_ENV, REPRO to match the cluster's paths.
#SBATCH --job-name=h5_act_l001
#SBATCH --nodes=1
...
# ── paths (edit these for the target cluster) ────────────────────────────────
...
NAME="h5_act_l001_12L_80000"
CKDIR="$REPRO/ckpt/$NAME"
mkdir -p "$CKDIR"
EVAL="$REPRO/eval_bank"
SEED=42

export ACT_LAMBDA=0.01 ACT_K_MAX=6 REINJECT_ALPHA=0.1 ACT_BASE_CKPT="$BASE_CKPT"
